...
public class LogParser {
    ...
    public Map<String, List<Entry>> parseLog(String log) {
         try(Scanner scanner = new Scanner(log)){
             ...
             while(scanner.hasNext()) {
                 ...
                 fillMap(elements, new Entry(entryDate, lineElements[POSITION_OF_IP], lineElements[POSITION_OF_LOGIN]));
             }
             return elements;
         }
    }
    ...
    private void fillMap(Map<String, List<Entry>> elements, Entry entry) {
        if(!elements.containsKey(entry.getIp())) {
            elements.put(entry.getIp(), new ArrayList<>());
        }
        elements.get(entry.getIp()).add(entry);
    }
