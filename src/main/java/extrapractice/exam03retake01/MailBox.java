...
public class MailBox {
    ...
    private List<Mail> findByContent(String substring) {
        List<Mail> result = new ArrayList<>();
        for (Mail mail : mails) {
            if (mail.getSubject().contains(substring) || mail.getMessage().contains(substring)) {
                result.add(mail);
            }
        }
        return result;
    ...
    private List<Mail> findByTo(String substring) {
        ...
        for (Mail mail : mails) {
            for (Contact contact : mail.getTo()) {
                if (contact.getName().equals(substring) ||
                        contact.getEmail().equals(substring)) {
                    result.add(mail);
                }
            }
        }
